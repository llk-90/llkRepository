...
; (function ($, window, document, undefined) {
    ...
    var cityObj = function (el, option) {
        ...
        this.defaultOptions = {
            ...
            dataUrl: baseURL+'/weixiplusBing/getAllSch.webapp',
        ...
        var dataHanlder = {
            ...
            splitData: function (data) {
                ...
                for (var provinceindex = 0;provinceindex<data.infolist.length;provinceindex++) {
                    //首先解析省份
                    var  tempprovince = data.infolist[provinceindex];
                    provinces[tempprovince.ProvinceId] = tempprovince.ProvinceName;
                    var cityList = [];
                    //解析城市
                    for (var cityindex = 0;cityindex<tempprovince.Citys.length;cityindex++){
                        var  tempCity = data.infolist[provinceindex].Citys[cityindex];
                        //解析区县
                        areas[tempCity.CityId] = tempCity.Areas;
                        cityList.push({"CityId":tempCity.CityId,"CityName":tempCity.CityName});

                    }
                    citys[tempprovince.ProvinceId] = cityList;
                }
        ...
        var loadDataHandler = {
            //加载省市区数据
            areaData: function () {
                //currentOptions.dataUrl,

                mui.ajax(baseURL+"/weixiplusBing/getAllSch.webapp",{
                    data:{
                        type:0
                    },
            ...
            townData: function (callback) {
                if (currentArea && currentArea.code) {
                    ...
                    $.ajax({
                        url: baseURL+'/weixiplusBing/getAllSch?type=1&TownId=' + currentArea.code + '.webapp',
                        dataType: 'json',
            ...
            liClick: function ($el, level) {  // lxj, 当前被点击的地址作为第一个实参被传入
                ...
                switch (level) {
                    ...
                    case 4://乡镇
                    {
                        ...
                        //setCookie("schoolName",currentTown.name,1);
                        //setCookie("schoolID",currentTown.code,1);
            ...
            createHtml: function (level,para) {  // lxj, para 在后面调用时，使用 town(一个对象，存储了全部乡镇街道) 实参传入
                switch (level) {
                    ...
                    case 2:
                    {
                        ...
                        //筛选已经选择的省下面的市
                        if (currentProvince && currentProvince.code) { // lxj，意味着这些城市是当前省下面的城市
                            for (var index = 0;index<citys[currentProvince.code].length;index++) {
                                var i = citys[currentProvince.code][index];
                                if (currentCity && currentCity.code && currentCity.code === i) {
                                    $city.append('<li city class="active" id="' + i.CityId + '" data-code="' + i.CityId + '">' + i.CityName + '</li>');
                                } else {
                                    $city.append('<li city id="' + i.CityId + '" data-code="' + i.CityId + '">' + i.CityName + '</li>');
                                }
                            }
                    ...
                    case 3:
                    {
                        ...
                        if (currentCity && currentCity.code) {
                            for (var index = 0;index<areas[currentCity.code].length;index++) {
                                var i = areas[currentCity.code][index];
                                if (currentArea && currentArea.code && currentArea.code === i) {
                                    $area.append('<li city class="active" id="' + i.TownId + '" data-code="' + i.TownId + '">' + i.TownName + '</li>');
                                } else {
                                    $area.append('<li city id="' + i.TownId + '" data-code="' + i.TownId + '">' + i.TownName + '</li>');
                                }
                            }
                    ...
                    {
                        if (para) {
                            for (var townindex = 0;townindex<para.infolist.length;townindex++) {
                                var i = para.infolist[townindex];
                                if (currentTown && currentTown.code && i === currentTown.code) {
                                    $town.append('<li city class="active" id="' + i.SchoolID + '" data-code="' + i.SchoolID + '">' + i.SchoolName + '</li>');
                                    $lastTown.text(para[i]);
                                } else {
                                    $town.append('<li city id="' + i.SchoolID + '" data-code="' + i.SchoolID + '">' + i.SchoolName + '</li>');
                                }
                                document.getElementById(i.SchoolID).addEventListener("tap",function(e){
                                    var targ = e.target;
                                    console.log(targ);
                                    document.getElementById("myschool").value=targ.innerHTML;
                                    SchoolID = e.srcElement.getAttribute("data-code");
 ...
 * Created by fengsn on 2018/3/21.
 */
