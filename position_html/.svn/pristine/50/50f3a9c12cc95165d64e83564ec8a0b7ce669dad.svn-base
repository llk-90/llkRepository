...
function pickerDateRange(inputId, options) {
	...
	var wrapper = {
		...
		ta: [
			'<div id="' + this.calendarId + '" class="ta_calendar ta_calendar2 cf" style="width: 96%;margin-left:2%">',
			'<div class="ta_calendar_cont cf" id="' + this.dateListId + '">',
			//'<table class="dateRangePicker"><tr id="' + this.dateListId + '"></tr></table>',
			'</div>',
			'<div style="display:none" class="ta_calendar_footer cf" ' + (this.mOpts.autoSubmit ? ' style="display:none" ' : '') + '>',
			'<div style="display:none" class="frm_msg">',
	...
	//备忘录
	/*$("#markList li").each(function(){
		$('#calendar_'+suffix+'_'+$(this).text()).attr('class',defaults.clickCss);
		alert($('#calendar_'+suffix+'_'+$(this).text()).text());
	});*/
};
...
pickerDateRange.prototype.init = function(isCompare) {
	...
	//备忘录
	$("#markList li").each(function() {
		$('#calendar_d_' + $(this).text()).attr('class', 'markday');
	});
};
...
pickerDateRange.prototype.show = function(isCompare, __method) {
	...
	$("#" + this.calendarId).css('top', pos.top + (__method.mOpts.theme == 'ta' ? 5 : 5) + 'px');
	//第一次显示的时候，一定要初始化输入框
...
pickerDateRange.prototype.fillDate = function(year, month, index) {
	...
	if(isTaTheme) {
		...
		if(this.mOpts.magicSelect) {
			...
			yh.push('</ul></div></div>');
			mh.push('</ul></div></div>');
			$(cap).append(mh.join('') + '<span class="joinLine"> 月 </span>' + yh.join(''));

		} else {
			$(cap).append((month + 1) + '月 ' + year);
		}

		$(table).append(cap);
		thead = document.createElement('thead');
		tr = document.createElement('tr');
		var days = ['周天', '周一', '周二', '周三', '周四', '周五', '周六'];
		for(var i = 0; i < 7; i++) {
	...
	for(var d = dateBegin; d.getTime() <= dateEnd.getTime(); d.setDate(d.getDate() + 1)) {
		if(d.getTime() < firstDayOfMonth.getTime()) { // 当前月之前的日期
			...
			tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
			deviation = '1';
		} else if(d.getTime() > today.getTime() && $('.workTime').html() != null) { // 当前时间之后的日期，或者开启统计之前的日期 d.getTime() >= today.getTime()
			tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
			deviation = '2';
		} else if(d.getTime() < today.getTime() && $('.workTime').html() == null && !(d.getDate() == dToday && d.getMonth() == mToday && d.getFullYear() == yToday)) { // 当前时间之前的日期，或者开启统计之前的日期 d.getTime() >= today.getTime()
			tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
			deviation = '2';
		} else { // 当前月日期
			deviation = '0';
