...
$.fn.longPress = function(fn) {
    ...
    for(var i = 0;i<$this.length;i++){
        ...
        $this[i].addEventListener('touchend', function(event) {
            clearTimeout(timeout);  //é•¿æŒ‰æ—¶é—´å°‘äºŽ800msï¼Œä¸ä¼šæ‰§è¡Œä¼ å…¥çš„æ–¹æ³•
            }, false);
    }
}
...
function getSize(id){
	...
	noneNodes = [],
	nodeStyle = [];
	getNoneNode(elem); //èŽ·å–å¤šå±‚displayï¼šnone;çš„å…ƒç´ 
	setNodeStyle();
   ...
   //è¿™æ–¹æ³•æ‰èƒ½èŽ·å–æœ€ç»ˆæ˜¯å¦æœ‰displayå±žæ€§è®¾ç½®ï¼Œä¸èƒ½style.displayã€‚
