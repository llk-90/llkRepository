...
@Service("userAnlysisService")
public class UserAnlysisService {
	...
	public Map<String, Object> list(PageData pd) throws Exception{
		...
		before = (int) dao.findForObject("UserAnlysisMapper.before", pd);
		if (pd.getString("timeUnit").equals(Const.DAY)) {
			...
			list = (List<PageData>) dao.findForList("UserAnlysisMapper.daylist", pd);
			if (!list.isEmpty()) {
				// 获取到日期集合
			...
			list = (List<PageData>) dao.findForList("UserAnlysisMapper.weeklist", pd);
			if (!list.isEmpty()) {
				// 获取到日期集合
				// 获取到日期集合
				String start = pd.getString("time_start"), end = pd.getString("time_end");
			...
			list = (List<PageData>) dao.findForList("UserAnlysisMapper.monthlist", pd);
			if (!list.isEmpty()) {
				// 获取到日期集合
				String start = pd.getString("time_start"), end = pd.getString("time_end");
