...
public class HeadController extends BaseController {
	...
	public Object sendSms() {
		...
		String PHONEs = pd.getString("PHONE"); // 对方电话
		...
		try {
			if (!PHONEs.isEmpty()) {
				...
				for (int i = 0; i < phone.length; i++) {
					if (Tools.checkMobileNumber(phone[i])) { // 手机号式不对就跳过
						SmsUtil.smsCodeSend(phone[i], CONTENT);
						String st=new String(phone[i]);
						phoneList.add(st);
						count++;
					} else {
						continue;
				...
				smsRecordService.insert_record(phoneList,CONTENT);				
				msg = "ok";
			}
		} catch (Exception e) {
			msg = "error";
