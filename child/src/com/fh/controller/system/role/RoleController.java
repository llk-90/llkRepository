...
public class RoleController extends BaseController {
	...
	public ModelAndView list(Page page)throws Exception{
			...
			if(roleId == null || "".equals(roleId)){
				pd.put("ROLE_ID", Const.ROLEADMIN);
			}
			List<Role> roleList = roleService.listAllRoles();				//列出所有部门
			page.setPd(pd);
			List<Role> roleList_z = roleService.listAllRolesByPId(page);		//列出此部门的所有下级
	...
	public Object hasR(){
		...
		try{
			pd = this.getPageData();
			if(roleService.findByRId(pd) != null){
				errInfo = "error";
			}
		} catch(Exception e){
			logger.error(e.toString(), e);
	...
	@ResponseBody
	public Object deleteRole(@RequestParam String ROLE_ID,Page page)throws Exception{
		Map<String,String> map = new HashMap<String,String>();
		PageData pd = new PageData();
		String errInfo = "";
		try{
