...
public class LoginController extends BaseController {
	...
	public ModelAndView toLogin() throws Exception {
		...
		mv.setViewName("system/admin/login_home");
		mv.addObject("pd", pd);
		return mv;
	}
	...
	/**
	 * 请求登录，验证用户
	 * 解密方法：先将解密字段分开然后分别调用Base64.decode()方法解密
	 */
	...
	public Object login() throws Exception {
		...
		if (null != KEYDATA && KEYDATA.length == 3) {
			...
			String code = Base64.decode(KEYDATA[2],"Unicode");
			//String code = KEYDATA[2];
			if (null == code || "".equals(code)) {
				errInfo = "nullcode"; // 验证码为空
			} else {
				String USERNAME = Base64.decode(KEYDATA[0],"Unicode");
				String PASSWORD = Base64.decode(KEYDATA[1],"Unicode");
...
//				if (Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)) {
					...
					if (pd != null) {
						...
						pageData.put("userId", pd.getString("USER_ID"));
						PageData list = userService.findCountByUserId(pageData);
						if (list != null && list.get("isLock").equals(1)) {
							// 被锁定
							errInfo = "locked"; // 账户被锁定
						} else {
							User user = new User();
							...
							user.setU_type(pd.get("u_type").toString());
							pd.put("LAST_LOGIN", DateUtil.getTime().toString());
							userService.updateLastLogin(pd);// 更新登录时间
							if (!(("").equals(pd.get("area_id")) || pd.get("area_id") == null)) {
								user.setFARM_ID((int) pd.get("area_id"));
							}
...
//				}
	...
	public ModelAndView login_index(@PathVariable("changeMenu") String changeMenu) {
		...
		try {
			...
			if (user != null) {
				...
				mv.setViewName("system/admin/login_home");// session失效后跳转登录页面
			}
	...
	public ModelAndView defaultPage(Page page) throws Exception {
		...
		String lastTime = user.getLAST_LOGIN();// 取得上次登录时间
		String lasIP = user.getIP();// 取得上次登录IP
		pd.put("USERNAME", USERNAME);

		pd = userService.findByUId(pd);
		mv.setViewName("system/admin/default");
		if ("".equals(lastTime) || lastTime == null || "".equals(lasIP) || lasIP == null) {
			pd.put("LAST_LOGIN", Const.TISHI);
			pd.put("IP", Const.TISHI);
		} else {
			pd.put("LAST_LOGIN", lastTime);
			pd.put("IP", lasIP);
		}
		mv.addObject("pd", pd);
		return mv;
