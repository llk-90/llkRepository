...
<div class="tab-content">
	...
	<form action="appuser/${msg }.do" name="userForm" id="userForm" method="post">
		...
		<table class="table ">
				...
				<%-- <tr>
					<td><label class="control-label" style="padding-top: 10px; width: 80px; float:right;"  for="loginname">用&nbsp;户&nbsp;名：</label></td>
					<td><input type="text" name="USERNAME" id="loginname" value="${pd.USERNAME }" placeholder="这里输入用户名" title="用户名"/></td>
				</tr>
				<tr>
					<td ><label class="control-label" style="padding-top: 10px; width: 80px; float:right;" >密&#12288;&#12288;码：</label></td>
					<td><input type="password" name="PASSWORD" id="password"   placeholder="输入密码"  title="密码"/></td>
				</tr>
				<tr>
					<td ><label class="control-label" style="padding-top: 10px; width: 80px; float:right;"  >确认密码：</label></td>
					<td><input type="password" name="chkpwd" id="chkpwd"   placeholder="确认密码"  title="确认密码"/></td>
				</tr>  --%>
					<%-- <tr>			
					<td ><label class="control-label" style="padding-top: 10px; width: 80px; float:right;" >所属区域：</label></td>
				
						<td ><select class="chzn-select" name="zd_id" id="ZD_ID"
									data-placeholder="请选择区域"
									style="vertical-align: top;"title="区域">
										<option value=""></option>
				...
				<tr>
					<td ><label class="control-label" style="padding-top: 10px; width: 80px; float:right;" >所属学校：</label></td>
							<td style="vertical-align: top;"><select class="chzn-select"
								name="ROLE_ID" id="role_id" data-placeholder="请选择学校"
				...
				<td ><label class="control-label" style="padding-top: 10px; width: 80px; float:right;">姓&#12288;&#12288;名：</label></td>
					<td><input type="text" name="NAME" id="name"  class="gbkOrName"  value="${pd.NAME }" placeholder="这里输入姓名" title="姓名" /></td>
				</tr>

				<tr>
					<td><label class="control-label" style="padding-top: 10px; width: 80px; float:right;">联系电话：</label></td>
				    <td><input type="tel" name="PHONE" id="PHONE" class="onlyNum" value="${pd.PHONE }" placeholder="这里输入手机号" title="手机号" onblur="hasPH('${pd.USERNAME }')"/></td>
				</tr>
				<tr>
				  				<td><label class="control-label" style="padding-top: 10px; width: 80px; float:right;">邮&#12288;&#12288;箱：</label></td>
				...
				<tr>
				  	<td><label class="control-label" style="padding-top: 10px; width: 80px; float:right;">备&#12288;&#12288;注：</label></td>
		...
		function ismail(mail){
			...
			//保存
			function save(){
				  if($("#password").val()!=""&&!$("#password").val().match(/^([a-zA-Z0-9!@#$%?_]){6,100}$/)){
			  		   $("#password").tips({
									side : 3,
			...
			function hasE(){
				 ...
				 $.ajax({
				    	...
				    	data: {EMAIL:EMAIL,user_id:user_id,tm:new Date().getTime()},
						dataType:'json',
						cache: false,
						success: function(data){
							 if("success" != data.result){
