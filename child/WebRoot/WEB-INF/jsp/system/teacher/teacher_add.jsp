...
<body>
	<form action="teacher/${msg }.do" name="Form" id="Form" method="post">
		...
		<div id="zhongxin">
			<table id="table_report" class="table table-striped table-bordered table-hover" style="margin-top: 30px">
				<tr>
					<td style="width: 70px; text-align: left; padding-top: 13px;">手&nbsp;&nbsp;机&nbsp;&nbsp;号：</td>
					<td>
						<input type="text" name="PHONE" id="PHONE" value="${pd.PHONE}" maxlength="13" placeholder="这里输入手机号" title="联系电话" />
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td style="width: 120px; text-align: left; padding-top: 13px;">所属学校：</td>
					<td>
				...
				<tr>
					<td style="width: 70px; text-align: left; padding-top: 13px;">入学年份：</td>
					<td>
						<select name="gradeParam" id="gradeParam" class="chzn-select" data-placeholder="请选择年份" onchange="getClass();">
							<option value=""></option>
						</select>
					</td>
				</tr>
				<tr>
					<td style="width: 70px; text-align: left; padding-top: 13px;">所属班级：</td>
					<td>
				...
				<tr>
					<td style="width: 70px; text-align: left; padding-top: 13px;">性&#12288;&#12288;别：</td>
					<td>
					...
					<td>
						<select name="ROLE_ID" id="ROLE_ID" class="chzn-select" data-placeholder="请选择角色" style="vertical-align: top;" title="角色">
							...
							<c:forEach items="${roleList }" var="var" varStatus="vs">
								<%-- <option value="${var.ROLE_ID }" <c:if test="${pd.ROLE_ID eq var.ROLE_ID}">selected="selected"</c:if>>${var.ROLE_NAME }</option> --%>
	...
	<script type="text/javascript">
		...
		function getClass(){
			...
			$.ajax({
				type:'POST',
				url:'<%=basePath%>teacher/classList.do',
						data : {
							Z_ID : Z_ID
						},
						dataType : 'json',
