...
<form action="StudentM/${msg}.do" name="Form" id="Form" method="post">
        <input type="hidden" name="z_parent_id" value="${pd.parentid}"/>
		...
		<table style="margin-left:5px;">
			...
			<tr>
				...
				<td><input name="z_name" id="z_name" placeholder="请输入名称" maxlength="8" title="名称" type="text"></td>
			</tr>
			<tr>
...
<!-- 						<select  class="chzn-select" name="z_type" id="z_type" data-placeholder="请选择节点类型"   onchange="tips();" > -->
<!-- 							<option value=""></option> -->
<%-- 							<option value="2"<c:if test="${pd.z_type  == '2' }">selected = "true"</c:if>>学校</option> --%>
<%-- 							<option value="3"<c:if test="${pd.z_type  == '3' }">selected = "true"</c:if>>入学年份</option> --%>
<%-- 							<option value="4"<c:if test="${pd.z_type  == '4' }">selected = "true"</c:if>>班级</option> --%>
<!-- 					    </select>		 -->
				    </td>
			</tr>
			<tr>
			    <td><label >排&#12288;&#12288;序：</label></td>
				<td><input style="margin-top: 6px"name="z_order" id="z_order" placeholder="请输入排序"  title="排序"  maxlength="20" type="text"></td>
			</tr>
...
// 			2016/9/2 yc 修改(增加z_parent_id参数)
			var z_parent_id= $('input[name="z_parent_id"]').val();
			var hasStuNo = false;
			$.ajax({
                type : "POST",
