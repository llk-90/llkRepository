	...
	<div class="container-fluid" id="main-container">
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<div class="row-fluid">
					<!-- 检索  -->
					<form action="eduInfoManage/list.do" method="post" name="educationInfoForm"
						...
						<table id="table_report"
							...
							<tbody>
								<!-- 开始循环 -->
								<c:choose>
									<c:when test="${not empty educationInfoList}">
										<c:forEach items="${educationInfoList}" var="educationInfoList" varStatus="educationInfo">
											<tr>
												<td class='center' style="width: 30px;">
													<label>
														<input type='checkbox' name='ids' value="${educationInfoList.id}" />
														<span class="lbl"></span>
													</label>
												</td>
												<td class='center' style="width: 30px;">${educationInfo.index+1}</td>
												<td class='center'>${educationInfoList.title }</td>
												<td class='center' style="width: 60%;">${educationInfoList.content }</td>
												<td class='center' ><fmt:formatDate value="${educationInfoList.create_time }" pattern="yyyy-MM-dd" /></td>
												<td class='center'>${educationInfoList.isTop }</td>
												<td class='center'>${educationInfoList.infoState }</td>
												<td style="width: 30px;" class="center">
                                                		<a class='btn btn-mini btn-info' title="修改" onclick="editE('${educationInfoList.id }');"><i class='icon-edit'></i></a>
                                                </td>
											</tr>
										</c:forEach>
									</c:when>
...
<script type="text/javascript" src="static/js/jquery.tips.js"></script>
	...
	<script type="text/javascript">
		...
		function search(){
			...
			var url='eduInfoManage/list.do';
			//alert(url);
			$("#educationInfoForm").attr('action',url);
			...
			if(str==''){
				...
				bootbox.confirm(msg, function(result) {
					if(result) {
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>eduInfoManage/deleteInfo.do?',
						    	data: {id:str},
								dataType:'json',
								cache: false,
		...
		function newInfo(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>eduInfoManage/newInfo.do';
			 diag.Width = 1000;
			 diag.Height = 800;
		...
		function editE(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>eduInfoManage/toEdit.do?id='+id;
			 diag.Width = 1000;
			 diag.Height = 800;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
