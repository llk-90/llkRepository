...
<body>
	<div class="container-fluid">
		<div id="page-content" class="clearfix">
			<form id="pageForm" name="pageForm"
				action="<%=basePath%>schoolAnnouncementController/list.do" method='post'>
				<table>
					<tr>
	...
	<script type="text/javascript">
		...
		function addAnno(){
			...
			diag.Drag=true;
			diag.Title ="发布";
			diag.URL = '<%=basePath%>schoolAnnouncementController/addAnno.do';
			diag.Width = 800;
			diag.Height = 600;
		...
		function editAnno(id){
			...
			diag.Drag=true;
			diag.Title ="编辑";
			diag.URL = '<%=basePath%>schoolAnnouncementController/editAnno.do?id='+id;
			diag.Width = 800;
			diag.Height = 600;
			diag.CancelEvent = function() { //关闭事件
				if (diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none') {
		...
		function delAnno(id){
			bootbox.confirm("确定删除该条公告么？",function(result){
				if(result){
					$("Form").attr("action","<%=basePath%>schoolAnnouncementController/delAnno.do?id="+id).submit();
				}
			});
		}
